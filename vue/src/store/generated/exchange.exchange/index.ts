...
import { Params } from "exchange-client-ts/exchange.exchange/types"
import { Pool } from "exchange-client-ts/exchange.exchange/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: exchange.exchange initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.ExchangeExchange.query.queryParams()).data
		...
		async QueryPool({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.ExchangeExchange.query.queryPool( key.id)).data
		...
		async QueryPoolAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.ExchangeExchange.query.queryPoolAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.ExchangeExchange.query.queryPoolAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgCreatePool({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.ExchangeExchange.tx.sendMsgCreatePool({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgUpdatePool({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.ExchangeExchange.tx.sendMsgUpdatePool({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgDeletePool({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.ExchangeExchange.tx.sendMsgDeletePool({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async MsgCreatePool({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.ExchangeExchange.tx.msgCreatePool({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreatePool:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgUpdatePool({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.ExchangeExchange.tx.msgUpdatePool({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgUpdatePool:Init Could not initialize signing client. Wallet is required.')
		...
		async MsgDeletePool({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.ExchangeExchange.tx.msgDeletePool({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgDeletePool:Init Could not initialize signing client. Wallet is required.')
